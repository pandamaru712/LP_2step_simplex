...
void read_one_line(FILE *fp,char *line){   //ファイルから１行ずつ読み込む
	/* ファイルから１行読み込む */
...
int read_one_token(char *string,double *num){   //Xの係数，添字解析
	int cnt=0;			        /* カウンタ */
	int x_num;			        /* Ｘの添字 */
...
void output_title(FILE *fout){   //タイトル出力
	int i,j;
	fprintf(fout,"２段階シンプレックス法\n");
	fprintf(fout,"目的関数\n Z =");
	for(i=1;i<=n;i++){
		fprintf(fout,"%+7.2fX%d",c[i],i);
	}
	if(max_min == max){
		fprintf(fout," ---> max");
	}else{
